...
> In the GUI, each argument is a separate tag. Press Enter after typing `--config`, then type the path and press Enter again.

**Variables & Secrets tab:** _(none needed)_
...
> The GUI mounts the entire bucket — do not specify a path in bucket. The config file will be accessible at `/mnt/litellm/litellm-config.yaml`.
